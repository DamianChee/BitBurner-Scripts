...
export async function main(ns) {
  ...
  const threadsThreshold = 0;

  while (true) {
    const ramAvailable = ns.getServerMaxRam(thisServer) - ns.getServerUsedRam(thisServer) - 32; // reserving 16gb for myself
    if (ramAvailable > 0) {
      ...
      ns.run("hack.js", 1, target, JSON.stringify(ms1));
      ns.run("weaken.js", wThreads1, target, JSON.stringify(ms2));
      ns.run("grow.js", gThreads ? gThreads : 1, target, JSON.stringify(ms3));
      ns.run("weaken.js", wThreads2, target, JSON.stringify(ms4));

      ns.printf(`Running batch: ETA ${ns.tFormat(wEndTime2 - Date.now())}`);
    }
    await ns.sleep(100);
  }
}
